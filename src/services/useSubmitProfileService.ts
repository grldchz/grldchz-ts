...
import { PostProfile, Profile, PostChangePassword } from '../types/Profile';
...
const useSubmitProfileService = () => {
  ...
  const changePassword = (postChangePassword: PostChangePassword) => {
    setService({ status: 'loading' });
    ...
    return new Promise((resolve, reject) => {
      fetch(process.env.REACT_APP_GRLDSERVICE_URL+'service.php', {
        method: 'POST',
        body: qs.stringify(postChangePassword),
        credentials: "include",
        headers
        })
        .then(response => {
  ...
  return {service, submitProfile, changePassword};
